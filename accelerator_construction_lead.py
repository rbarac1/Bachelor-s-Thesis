...
def create_particles(N, v_ave, v_dev, part="ion", acc=linac, C=0, A=0):
...
def vtest(v_ave, v_dev, N=1e3, part="ion", acc=linac, C=0, A=0):
    acc.reset()
...
def testrun(vtest, N_electrodes, part="ion", acc=linac, C=0, A=0):
    if part=="proton":
...
def create_test_particle(vtest, part="ion", acc=linac, C=0, A=0):
    if part=="proton":
        acc.add_particle(lc.proton([0, 0, vtest]))
    elif part=="ion":
        acc.add_particle(lc.ion([0, 0, vtest],C,A))
...
vt = 0.08*lc.c
v = vt
v = vtest(vt, vt/(1e2), part="ion", acc=linac, C=53, A=208)
...
testrun(v, 10, part="ion", acc=linac, C=53, A=208)
print("mass and charge:", linac.beam[0].m, linac.beam[0].q)
