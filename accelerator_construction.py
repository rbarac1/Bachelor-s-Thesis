...
def create_particles(N, v_ave, v_dev, part="ion", acc=linac, C=1, A=1):
    for i in range(int(N)):
        v = np.random.normal(v_ave, v_dev)     #speed
        theta = np.random.random()*1*np.pi/180     #polar angle
...
def vtest(v_ave, v_dev, N=1e3, part="ion", acc=linac, C=1, A=1):
    acc.reset()
...
def testrun(vtest, N_electrodes, part="ion", acc=linac, C=1, A=1):
    if part=="proton":
...
linac.add_segment(lc.collimator(1.0, 1e-3))
vt = 0.08*lc.c
v = vt
v = vtest(vt, vt/(1e3), part="ion", C=-1, A=1)
print("mass and charge:", linac.beam[0].m, linac.beam[0].q)
...
testrun(v, 120, part="ion", C=-1, A=1)
print("mass and charge:", linac.beam[0].m, linac.beam[0].q)
